...
export async function POST(req: NextRequest) {
  ...
  for (const exercise of body.exercises) {
    const { data: exerciseData, error: exerciseError } = await (
      ...
      .single();
    if (exerciseError)
      return NextResponse.json({ error: exerciseError }, { status: 500 });
    ...
    const { error: setError } = await (await supabase)
      ...
      .insert(setsToInsert);
    if (setError)
      return NextResponse.json({ error: setError }, { status: 500 });
  }

  return NextResponse.json(
    { message: "The data has been received", received: body, userId: user.id },
    { status: 201 },
  );
}
